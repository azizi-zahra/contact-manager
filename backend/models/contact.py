...
class Contact(SQLModel, table=True):
    ...
    family_name: Optional[Annotated[str, StringConstraints(max_length=50)]] = Field(default=None, nullable=True)
    phone: Annotated[str, StringConstraints(max_length=15, pattern=r'^09\d{9}$')]
    email: Optional[Annotated[str, StringConstraints(max_length=100)]] = Field(default=None, nullable=True)
    city: Optional[Annotated[str, StringConstraints(max_length=50)]] = Field(default=None, nullable=True)
    user_id: int = Field(foreign_key="user.id")
    
    user: "User" = Relationship(back_populates="contacts")
    labels: List["Label"] = Relationship(back_populates="contacts", link_model=ContactLabel)
